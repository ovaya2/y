...
| UC-11 | DSGVO-Löschung | Personendaten in `m_gl` (vorname, nachname, geburtsdatum, email, telefon, iban, bic) anonymisieren; `m_adresse` bleibt unberührt (gebäudebezogen) |
...
| DSGVO-Löschung | Personendaten in `m_gl` anonymisieren, `m_adresse` bleibt | AO §147 (10 Jahre Beitragsdaten); Adresse ist gebäudebezogen |
...
| 4 | Post-Deployment-Skript für Seed-Daten (`d_perm`, `d_role`, `d_anr`, `d_sta`, …) | 1 h |
